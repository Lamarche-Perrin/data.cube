...
plot.data.data.cube <- function (dc, data='obs', rank=NULL, display=NULL, sep.dim=NULL) {
    ...
    if (is.null (sep.dim)) {
        ...
        p <- ggplot (df, aes(x=label, y=data)) +
            geom_bar (aes (fill=topic), position="dodge", stat="identity")
    }
...
plot.outliers.data.cube.backup <- function (dc, poisson=FALSE, threshold=3, labels=FALSE, min.obs=NULL) {
    div.mean <- mean (dc$data$div)
    div.sd <- sd (dc$data$div)
    dc$data$outlier <- findInterval (dc$data$div, div.mean + div.sd * threshold * c(-1,1)) - 1
    dc$data$outlier.shape <- ifelse (dc$data$outlier == 0, 'normal', 'abnormal')
    
    df <- as.data.frame (dc$data)
    lapply (dc$dim.names, function (d) df[[d]] <<- dc$elem.names[[d]][df[[d]]])
    df$name <- apply (df, 1, function (row) paste (row[dc$dim.names], collapse=' '))

    if (! is.null (min.obs)) { df <- df[df$obs >= min.obs,] }
    
    if (FALSE && !poisson) { 
        obs <- 10 ^ seq (log10 (1), log10 (max (dc$data$obs, dc$data$exp))+1, 0.01)
        out <- data.frame (obs)
        out$pos <- out$obs / 2 ^ ((div.mean + threshold * div.sd) * dc$sum / out$obs)
        out$neg <- out$obs / 2 ^ ((div.mean - threshold * div.sd) * dc$sum / out$obs)
        out$name <- NA
    }

    p <- ggplot (data=df, aes (x=obs, y=obs/exp)) +
        scale_x_log10 () + scale_y_log10 () +
        geom_point (aes (size=abs(div), fill=div, shape=outlier.shape)) +
        scale_shape_manual (values=c(22,21)) +
        scale_fill_gradient2 (low='blue', high='red') +
        theme_bw () + theme (text=elem_text (size=20))

    if (labels) { p <- p + geom_text_repel (data=df[df$outlier != 0,], aes (x=obs, y=obs/exp, label=name)) }
    
    if (FALSE && !poisson) {
        p <- p + geom_path (data=out[out$pos > 0,], aes (x=pos, y=obs)) +
            geom_path (data=out[! is.infinite(out$neg),], aes (x=neg, y=obs))
    }
...
## Print data.cube summary
summary.data.cube <- function (dc) {
    ## TODO: print 3 levels of the cube
    cat ("Datacube with", dc$dim, ifelse (dc$dim > 1,"dims:","dim:"), "\n")
    for (d in names(dc$names)) { cat ("-> '", d, "' contains ", dc$elem.nb[[d]], " elem", ifelse (dc$elem.nb[[d]] > 1, "s", ""), "\n", sep='') }
    cat ("-> total: ", length(dc$data$obs), " non-empty elems among ", prod(unlist(dc$elem.nb)), " elem", ifelse (prod(unlist(dc$elem.nb)) > 0, "s", ""), " (density = ", length(dc$data$obs)/prod(unlist(dc$elem.nb)), ")\n", sep='')
    cat ("totalling", dc$sum, "units:\n")
    ## for (d in names(dc$names)) {
    ##     cat ("-> distribution within '", d, "':\n", sep='')
    ##     print (summary (dc$margins[[d]]$obs))
    ## }
    ## cat ("-> distribution within the whole datacube:\n", sep='')
    summary(dc$data$obs)
}


## Remove elems
remove.tail.elems <- function (obj, ...) { UseMethod ('remove.tail.elems') }
remove.tail.elems.data.cube <- function (dc, n) {
    elems <- list()
    for (d in seq_along (n)) {
        rem <- n[[d]]
        dim <- names(n)[[d]]
        elems[[dim]] <- dc$elem.names[[dim]] [dc$margins[[dim]]$dim [order (dc$margins[[dim]]$obs) [1:rem]]]
    }
    return (remove.elems (dc, elems))
}


## Extract elems
select.head.elems <- function (obj, ...) { UseMethod ('select.head.elems') }
select.head.elems.data.cube <- function (dc, n) {
    elems <- list()
    for (d in seq_along (n)) {
        ext <- n[[d]]
        dim <- names(n)[[d]]
        elems[[dim]] <- dc$elem.names[[dim]] [dc$margins[[dim]]$dim [order (-dc$margins[[dim]]$obs) [1:ext]]]
    }
    return (select.elems (dc, elems))
}


## Aggregate temporal dim
aggregate.time <- function (obj, ...) { UseMethod ('aggregate.time') }
aggregate.time.data.cube <- function (dc, level='day', dim='time') {
    if (level == 'day') { return (dc) }
    if (level == 'week') {
        day2week <- function (day) { as.character (7 * floor (as.numeric (as.Date (day)-1+4) / 7) + as.Date (1-4, origin = "1970-01-01")) }
        days <- dc$elem.names[[dim]]
        weeks <- day2week (days)
        elems <- list()
        elems[[dim]] <- list()
        for (week in unique (weeks)) { elems[[dim]][[week]] = days[weeks == week] }

        aggregate.elems (dc, elems)
    }
}


## Head of obs values
head.obs <- function (obj, ...) { UseMethod ('head.obs') }
head.obs.data.cube <- function (dc, n=6, dim=NULL) {
    if (is.null (dim)) {
        if (n == 0) { head <- as.data.frame (dc$data) [order (-dc$data$obs), ] }
        else { head <- as.data.frame (dc$data) [order (-dc$data$obs)[1:n], ] }
        for (d in dc$dim.names) { head[[d]] <- dc$elem.names[[d]][head[[d]]] }
    }

    else {
        if (n == 0) { head <- as.data.frame (dc$margins[[dim]]) [order (-dc$margins[[dim]]$obs), ] }
        else { head <- as.data.frame (dc$margins[[dim]]) [order (-dc$margins[[dim]]$obs)[1:n], ] }
        head$dim <- dc$elem.names[[dim]][head$dim]
    }
...
## Head of div values
head.div <- function (obj, ...) { UseMethod ('head.div') }
head.div.data.cube <- function (dc, n=6) {
    if (n == 0) {
        head <- as.data.frame (dc$data) [order (-dc$data$div), ]
        for (d in dc$dim.names) { head[[d]] <- dc$elem.names[[d]][head[[d]]] }
        row.names (head) <- NULL
    }

    else {
        head <- as.data.frame (dc$data) [order (-dc$data$div)[1:n], ]
        for (d in dc$dim.names) { head[[d]] <- dc$elem.names[[d]][head[[d]]] }
        row.names (head) <- NULL
    }
...
## Plot head of obs values
plot.head.obs <- function (obj, ...) { UseMethod ('plot.head.obs') }
plot.head.obs.data.cube <- function (dc, n=6) {
    data <- head.obs (dc, n)
    data$label <- apply (data, 1, function (row) paste (row[1:dc$dim], collapse=' '))
    data$label <- factor (data$label, levels=unique(data$label))
    ggplot (data, aes (x=label, y=obs)) +
        geom_bar (stat="identity") +
        theme_bw () +
        theme (axis.text.x = elem_text (angle = 90, hjust = 1)) +
        theme (text=elem_text (size=20))
}
